...
TEnv = Dict[str, Callable | Tuple | type | Dataclass]
...
def typecheck(program: Program) -> Program:
    ...
    def tc_exp(e: Expr, env: TEnv) -> Callable | Tuple | type | Dataclass:
        match e:
            case FieldRef(lhs, field):
                assert isinstance(tc_exp(lhs, env), Dataclass)
                return env[field]
            ...
            case Var(x):
                if x in global_values:
                    ...
                    elif isinstance(env[x], Dataclass):
            ...
            case Call(func, args):
                ...
                match tc_exp(func, env):
                    case Callable(param_types, return_type):
                        ...
                        return return_type
                    case Dataclass(params):
                        i = 0
                        for param in params.values():
                            assert param == arg_types[i]
                            i += 1
    ...
    def tc_stmt(s: Stmt, env: TEnv):
        match s:
            case ClassDef(name, superclass, args):
                arg_dict: dict[str, type] = {}
                for arg in args:
                    env[arg[0]] = arg[1]
                    arg_dict[arg[0]] = arg[1]
                dataclass_names.add(name)
                env[name] = Dataclass(arg_dict)  # TODO: does this work?
...
def rco(prog: Program) -> Program:
    ...
    def rco_stmt(stmt: Stmt, bindings: Dict[str, Expr]) -> Stmt:
        match stmt:
            case ClassDef(name, superclass, args):
                return ClassDef(name, superclass, args)
            case FunctionDef(name, params, body_stmts, return_type):
    ...
    def rco_exp(e: Expr, bindings: Dict[str, Expr]) -> Expr:
        match e:
            case FieldRef(lhs, field):
                exp = rco_exp(lhs, bindings)
                return FieldRef(exp, field)
            case Var(x):
...
# translation
##################################################
def translation(prog: Program) -> Program:
    def translate_exp(e: Expr, bindings: Dict[str, Expr]) -> Expr:
        match e:
            case Prim(op, args):
                new_args = [translate_exp(e, bindings) for e in args]
                new_e = Prim(op, new_args)
                new_v = gensym('tmp')
                bindings[new_v] = new_e
                return Var(new_v)
            ...
            case FieldRef(lhs, field):
                index = -1
                i = 0
                for key in dataclass_var_types[lhs.name].args:
                    if key == field:
                        index = i
                    i += 1
                if index == -1:
                    raise Exception('translate_exp', e)
                new_e = Prim('subscript', [lhs, Constant(index)])
                new_v = gensym('tmp')
                bindings[new_v] = new_e
                return Var(new_v)
            case Var(x):
                if x in function_names:
                    ...
                    bindings[new_v] = Var(x)
                    return Var(new_v)
                # elif x in dataclass_var_types:
                #     return dataclass_var_types[x]
                else:
                    return Var(x)
            case Constant(i):
                return Constant(i)
            case Call(func, args):
                if func.name in dataclass_names:
                    new_args = [translate_exp(e, bindings) for e in args]
                    return Prim('tuple', new_args)
                else:
                    new_args = [translate_exp(e, bindings) for e in args]
                    new_func = translate_exp(func, bindings)
                    new_e = Call(new_func, new_args)
                    new_v = gensym('tmp')
                    bindings[new_v] = new_e
                    return Var(new_v)
                # return Prim('tuple', [func, args])
            case _:
                raise Exception('rco_exp', e)

    def translate_stmt(stmt: Stmt, bindings: Dict[str, Expr]) -> Stmt:
        match stmt:
            case FunctionDef(name, params, body_stmts, return_type):
                return FunctionDef(name, params, translate_stmts(body_stmts), return_type)
            case Assign(x, e1):
                new_e1 = translate_exp(e1, bindings)
                if isinstance(new_e1, Prim):
                    if new_e1.op == 'tuple':
                        tmp = dataclass_var_types[x]
                        tuple_var_types[x] = tuple(tmp.args[key] for key in tmp.args)
                return Assign(x, new_e1)
            case If(cond, then_stmts, else_stmts):
                return If(translate_exp(cond, bindings), translate_stmts(then_stmts), translate_stmts(else_stmts))
            case While(Begin(s1, cond), s2):
                return While(Begin(translate_stmts(s1), translate_exp(cond, bindings)), translate_stmts(s2))
            ...
            case _:
                raise Exception('translate_stmt', stmt)
    def translate_stmts(stmts: List[Stmt]) -> List[Stmt]:
        new_stmts = []
        bindings = {}
        ...
        for stmt in stmts:
            if not isinstance(stmt, ClassDef):
                # (1) compile the statement
                new_stmt = translate_stmt(stmt, bindings)
                # (2) add the new bindings created by rco_exp
                new_stmts.extend([Assign(x, e) for x, e in bindings.items()])
                # (3) add the compiled statement itself
                new_stmts.append(new_stmt)
    ...
    return Program(translate_stmts(prog.stmts))
